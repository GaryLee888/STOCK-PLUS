...
class DayTradeWorker:
    ...
    def login_and_prepare(self):
        ...
        if not API_KEY or not SECRET_KEY:
            print("❌ 錯誤：無法讀取 API_KEY 或 SECRET_KEY，請檢查 GitHub Secrets 設定。")
            return False
        try:
    ...
    def create_card(self, item):
        # 搜尋 Linux 系統安裝的中文字體路徑
        potential_fonts = [
            "/usr/share/fonts/truetype/wqy/wqy-microhei.ttc", # YAML 安裝的路徑
            "msjhbd.ttc",                                    # 備用檔案
        ...
        draw.rectangle([0, 0, 15, 400], fill=(255, 60, 60)) # 左側紅條
    ...
    def start_monitoring(self):
        ...
        while True:
            ...
            time.sleep(15) # 掃描頻率 15 秒

        # 任務結束，產出 Excel 報表
        if self.results:
            df = pd.DataFrame(self.results)
            output_file = get_daily_filename()
            df.to_excel(output_file, index=False)
            print(f"✅ 今日交易紀錄報表已存至: {output_file}")
